...
\begin{cases} 
0 & \text{se } x_n \text{ è sul lato corretto del margine} \\
...
\end{cases}$$
	...
	=> questo mi dice => se mi classifichi correttamente non paghi nulla (vince lo zero) => viceversa se ottieni un valore negativo paghi una penalità sempre più alta a seconda del valore che ottieni... oltre zero pago penalità molto alta!
...
- Questo merita considerazione:
	...
	=> L'hinge loss non contribuisce in alcun modo per gli esempi classificati correttamente! => per questo motivo risulta robusto agli outliers! => se ho un punto lontano dal margine (anche molto) ma classificato correttamente questo ha loss nulla!
...
- Tutto ciò di cui abbiamo realmente bisogno è $\langle \phi(\mathbf{x}_n), \phi(\mathbf{x}_m) \rangle$. (ovvero solo del loro prodotto interno??) => esattamente solo del prodotto scalare!
...
- Quindi,   $k(x,z) = (x^Tz)^2$  corrisponde all'**embedding polinomiale** $\phi(x) = (x_1^2, \sqrt{2} x_1 x_2, x_2^2)$ => tutti termini di secondo grado...

	16
---
**Costruire kernel**

- Possiamo combinare kernel in modi che **preservino la semidefinitezza positiva**: Tramite kernel function possiamo implicitamente mappare i nostri dati in spazi a più dimensioni! => tutti quelle funzioni che perservano la semidefinitezza positiva sono appunto denominate kernel
...
- Un kernel molto comunemente usato è il Gaussiano(è l'RBF in scikit *spoiler*): $$k(x, z) = \exp\left\{-\frac{\|x - z\|^2}{2\sigma^2}\right\}$$
...
- E al nostro classificatore: $$f(x) = \sum_{n=1}^N a_n y_n k(x, x_n) + b$$$$= \sum_{z \in SV} a_n y_n k(x, z) + b$$ => notare si continui a usare ==solo i vettori di supporto== per ottenere il nostro classificatore => lo spazio dello soluzioni dunque non esplode nel caso dei kernel!
...
- Il kernel trick ci permette di definire un problema di apprendimento lineare (in uno spazio *potenzialmente **infinito dimensionale***) che è non lineare nello spazio originale. => ==senza dover mai calcolare esplicitamente le mappe di feature per tutti i punti!== => l'unico costo è calcolare $k(x,z)$
